...
class SchemaValue[T]:
    ...
    """
    def __init__(self, export_doc:str, export_value:Callable[[T], Optional[str]], convert_input:Callable[[BaseType], Optional[T]]):
    ...
    def path_schema():
        ...
        return SchemaValue(export_doc='"<path>"', export_value=SchemaValue.export_path, convert_input=SchemaValue.convert_path)
...
class _SchemaItem[T]:
    ...
    def help_str(self, tab:str='  '):
        ...
        s = f'\n\n{tab}'.join([self.usage_str(), self.description, f'Default: {self.export_value(self.default_value)}'])
        return s
...
class _SchemaTable:
    ...
    def add_item(self, name:str, possible_values:list[SchemaValue], default, description:str):
        """Add schema item"""
    ...
    def help_str(self, level:int=0) -> str:
        ...
        top_str = '\n'.join(top_str)
        sub = '\n\n'.join([x.help_str(level=level+1) for x in self.__subtables.values()])
        v = '\n\n'.join([x.help_str() for x in self.__data.values()])
        return f'{top_str}\n\n{v}\n\n{sub}'

    def parse_data[T](self, data, namespace:Optional[T|Namespace]=None) -> T | Namespace:
        ...
        for key, schema in self.__data.items():
            value = schema.convert_input(data.pop(key)) if key in data else schema.default_value
    ...
    def format_export_keys(self, namespace, *keys, use_fullname:bool=False) -> str:
        ...
        for key in keys:
            if key in self.__data:
                ...
                lhs = f'{self.__full_name}.{schema.short_name}' if use_fullname and self.__full_name else f'{schema.short_name}'
...
class Schema(_SchemaTable):
    ...
    def load_toml[T](self, filepath:pathlib.Path, namespace:Optional[T|Namespace]=None) -> T|Namespace:
